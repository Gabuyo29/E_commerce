...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($sizes as $index => $size) {
        ...
        if ($stmt->fetch()) {
            ...
            $insert_stmt->bind_param("sssdsisssi", $product_id, $name, $description, $price, $category, $stock, $size, $image_path, $username, $user_id);
            $insert_stmt->execute();
            $insert_stmt->close();
        }
    }
...
<body>
    ...
    <form action="add_product_new.php" method="POST" enctype="multipart/form-data">
        ...
        <select id="category" name="category" required>
            ...
            <option value="Cotton_Sleeves" <?php echo isset($_POST['category']) && $_POST['category'] === 'cotton_sleeves' ? 'selected' : ''; ?>>Cotton Sleeves</option>
            <option value="Mens_Wear" <?php echo isset($_POST['category']) && $_POST['category'] === 'mens_wear' ? 'selected' : ''; ?>>Men's Wear</option>
            <option value="Sports_Wear" <?php echo isset($_POST['category']) && $_POST['category'] === 'sports_wear' ? 'selected' : ''; ?>>Sports Wear</option>
        </select>
        ...
        <button type="button" onclick="addSizeStockRow()">Add More Sizes</button>
